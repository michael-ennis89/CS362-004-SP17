...
void randomTestCardGreatHall()
{
    ...
    int cardsNeeded = numHandCards(testGameState);

    /*Number of action player should have after Great Hall card is played*/
    int actionNeeded = testGameState->numActions;
    actionNeeded = actionNeeded + 1;

    /*Play the GreatHall Card*/
    //greatHall_Card(0, currentPlayer,testGameState);
    great_hallPlay(currentPlayer, testGameState, 0); //Re-factored for Travis's Code

    /*Check how many cards & actions the current player has */
    int cardsHave = numHandCards(testGameState);
    ...
    else
    {
        ...
        if(cardsNeeded != cardsHave)
        {
            ...
            printf("%d ", cardsNeeded);
            printf("%d \n", cardsHave);
            if(actionNeeded != actionHave)
            {
                printf(" also Incorrect number of actions ");
                printf("%d ", actionNeeded);
                printf("%d ", actionHave);
            }
        }
        else if(actionNeeded != actionHave)
        {
            ...
            if(cardsNeeded != cardsHave)
            {
                printf(" also Incorrect number of cards ");
                printf("%d ", cardsNeeded);
                printf("%d ", cardsHave);
            }
        }
